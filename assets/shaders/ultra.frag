...
float noise(vec3 x)
{
    ...
    return mix(
        mix(mix(hash(i+ivec3(0,0,0)), hash(i+ivec3(1,0,0)),f.x),
...
vec2 scene(vec3 p)
{
    ...
    for(int o = 0; o < stars; o++)
    {
        ...
        sce = opUnion(
            sce,
            sdRoundCone(q,vec3(0.),vec3(1.,1.,0.),.1,.01,4.)
        );
    }
...
vec3 shade(vec2 t,vec3 rd,vec3 ro)
{
    ...
    if(t.y == 0.)
    {
        col = mix(vec3(0.702,0.,0.)*.75,
                  vec3(0.969,0.,0.)*.75,
                  smoothstep(0.3,.0,rd.y));
    }
    else if(t.y == 1.)
    {
