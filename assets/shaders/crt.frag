...
// make tile piece
float truchet(vec3 p, vec3 x, vec2 r) { return min(torus(p - x, r), torus(p + x, r)); }
// const
...
vec3 normal(vec3 p, float t)
{
  ...
  vec3  n = h.xyy * map(p + h.xyy * e).x + h.yyx * map(p + h.yyx * e).x +
           h.yxy * map(p + h.yxy * e).x + h.xxx * map(p + h.xxx * e).x;
  return normalize(n);
}
...
vec4 render(inout vec3 ro, inout vec3 rd, inout vec3 ref, bool last, inout float d)
{
  ...
  for (int i = 0; i < 150; i++)
  {
    p        = ro + rd * d;
    vec2 ray = map(p);
    if (abs(ray.x) < MIN_DIST * d || d > MAX_DIST)
      break;
  ...
  if (d < MAX_DIST)
  {
    ...
    if (m == 1.)
    {
      ...
      // coord checker
      float tm = T * .25;
      vec2 tuv = vec2(fract(1. * chk * angle2 / 1.57 + tm), (d - (.5 - width)) / (2. * width) * 2.);
      tuv.y -= .5;
      vec2 tid = vec2(floor(1. * chk * angle2 / 1.57 + tm), floor(d - (.5 - width)) / (2. * width));
...
void mainImage(out vec4 O, in vec2 F)
{
  ...
  vec3 ro = uCameraPos;
  vec3 rd = normalize(vec3(uv * uZoom, -1.0));
  // mouse rotation
  float x = -.305;
  float y = -.750;
