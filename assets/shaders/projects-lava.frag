...
{
  t = clamp(t, 0.0, 1.0);
  vec3 c = mix(vec3(0.02, 0.05, 0.16), vec3(0.0, 0.35, 0.65), smoothstep(0.0, 0.40, t));
  c = mix(c, vec3(0.35, 0.80, 1.0), smoothstep(0.40, 0.72, t));
  c = mix(c, vec3(0.92, 0.98, 1.0), smoothstep(0.72, 1.0, t));
  return c;
}
...
void main()
{
  ...
  if (esc > 0.5)
  {
    ...
    col += vec3(0.7, 0.9, 1.0) * smoothstep(0.6, 1.0, abs(sin(sn * 0.7))) * 0.12;  // frost layers

    float frost = smoothstep(0.6, 1.0, t);
    float spk   = step(0.94, hash21(floor(z * 40.0) + floor(uTime * 3.0)));
    col += vec3(1.0) * spk * frost * 0.5;              // sparkles
  }
  else
  {
    col = vec3(0.015, 0.03, 0.09);                     // frozen core
    col += vec3(0.05, 0.16, 0.32) * (0.5 + 0.5 * sin(uTime * 0.5)) * 0.2;
    col *= 0.9 + 0.2 * detail(z * 5.0);
  }
