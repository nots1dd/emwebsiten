// Blog — pixel dithered-gradient background
...
/* gradient brightness in [0,1] for a pixel block */
...
/* pixel '+' trail in a cool neon palette (cyan <-> violet) that contrasts the
   blog's indigo theme, with a white-hot core where the bars cross */
void cursorTrail(inout vec3 col, vec2 scr, float aspect)
{
  ...
  for (int i = 0; i < 16; i++)
  {
    ...
    vec3  tc = mix(vec3(0.15, 0.90, 1.00), vec3(0.60, 0.30, 1.00), m);   // cyan <-> violet
    tc = mix(tc, vec3(1.0), hbar * vbar * 0.85);                         // white-hot center
    col = mix(col, tc, plus * life * taper);
  }
}
