// this is a modified shader of Seascape written by
...
const vec3 SEA_BASE        = vec3(0.0, 0.02, 0.06);
const vec3 SEA_WATER_COLOR = vec3(0.18, 0.32, 0.45) * 0.55;
...
vec3 getSkyColor(vec3 e)
{
  ...
  vec3 nightSky =
    vec3(pow(1.0 - e.y, 2.8) * 0.08, pow(1.0 - e.y, 1.8) * 0.12, 0.18 + (1.0 - e.y) * 0.25);

  /* subtle moonlight tint */
  nightSky += vec3(0.02, 0.03, 0.05);
...
vec3 getPixel(vec2 fragCoord)
{
  ...
  vec3 light = normalize(vec3(-0.2, 0.65, 0.35));
